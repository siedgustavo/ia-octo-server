...
- `prometheus`: scrapes `octofan-controller:8000/metrics`, vLLM metrics and node exporter.
...
- `vllm`: OpenAI-compatible inference container on the same Docker network when AI metrics are enabled.
...
7. Prometheus scrapes the controller, vLLM and node exporter metrics.
...
10. The LED loop maps AI runtime health and NVIDIA activity to front-panel LEDs through `fan_controller_cli -l`.
...
The compose stack uses the Docker network `octofan-ai` by default. This gives the controller a stable way to reach the vLLM container at `http://vllm:8000`.

If another compose project owns vLLM, connect the container or its compose network to `octofan-ai` and update `ai.base_url`.
