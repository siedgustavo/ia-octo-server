...
6. Prometheus scrapes those metrics and Grafana visualizes them.
7. The OLED loop renders an 8-line, 20-character layout through `fan_controller_cli -o`.
8. The watchdog loop runs configured checks and feeds the watchdog with `fan_controller_cli -s` only when healthy.
