...
This spreads each model across both GPUs, quarters KV-cache memory relative to `f16`, and keeps idle models resident. Both local models use 128k context windows with every layer on GPU. The local Ollama image is built from 0.32.5 with a one-line scheduler patch that uses all reported free VRAM instead of evicting at 80%; the official CUDA and llama-server libraries remain unchanged. When another model truly needs memory, Ollama queues the request and unloads idle models as necessary. API callers can override the policy per request with `keep_alive`.
